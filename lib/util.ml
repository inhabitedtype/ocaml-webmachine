...
module Ifs = struct
  ...
  module Make(H:sig val header : string end) = struct
    let get req =
      Header.get req.Request.headers H.header
    ...
    let matches_etag req etag =
      match etag with
      | None      -> true
      | Some etag ->
        List.mem etag (ETag.from_header (get req))
  end

  module Match = Make(struct let header = "if-match" end)
  module None_match = Make(struct let header = "if-none-match" end)
  module Unmodified_since = struct
    include Make(struct let header = "if-unmodified-since" end)

    let is_valid_date req =
      (* XXX(seliopou): NYI, do it eventually *)
      false

    let compare req date =
      (* XXX(seliopou): NYI, do it eventually *)
      0
  end
  module Modified_since = struct
    include Make(struct let header = "if-modified-since" end)
